...
const incidentLayerIds = {
  flood: ['depth'],
  sectors: ['incident-areas-fill', 'incident-areas-line', 'incident-areas-label'],
...
function setIncidentLayers(layers = {}, selected) {
  ...
  if (map?.getLayer('incident-areas-line')) {
    map.setPaintProperty('incident-areas-line', 'line-color', ['case', ['==', ['get', 'id'], selected], '#7fd7ff', '#d39152']);
    map.setPaintProperty('incident-areas-line', 'line-width', ['case', ['==', ['get', 'id'], selected], 3.5, 1.5]);
  }
}
...
async function addIncidentLayers() {
  try {
    ...
    map.addSource('incident-hazards', { type: 'geojson', data: pointFeatures(fixture.sectors.filter((s) => s.severity === 'Critical'), (s) => ({ label: `Hazard · ${s.code}` })) });
    map.addSource('incident-people', { type: 'geojson', data: pointFeatures(fixture.sectors.filter((s) => s.people?.[1]), (s) => ({ label: `${s.people[0]}–${s.people[1]} people · ${s.code}` })) });
    map.addSource('incident-landmarks', { type: 'geojson', data: pointFeatures(fixture.places, (p) => ({ label: p.name })) });
    map.addSource('incident-flow', { type: 'geojson', data: { type: 'Feature', properties: {}, geometry: { type: 'LineString', coordinates: fixture.river.reduce((out, v, i, a) => i % 2 ? out : out.concat([[v, a[i + 1]]]), []) } } });
    map.addLayer({ id: 'incident-flow', type: 'line', source: 'incident-flow', paint: { 'line-color': '#36a7d2', 'line-width': 4, 'line-opacity': 0.72, 'line-dasharray': [1, 1.6] } });
    map.addLayer({ id: 'incident-areas-fill', type: 'fill', source: 'incident-areas', paint: { 'fill-color': '#d39152', 'fill-opacity': 0.12 } });
    map.addLayer({ id: 'incident-areas-line', type: 'line', source: 'incident-areas', paint: { 'line-color': '#d39152', 'line-width': 1.5 } });
    map.addLayer({ id: 'incident-areas-label', type: 'symbol', source: 'incident-areas', layout: { 'text-field': ['get', 'code'], 'text-size': 12, 'text-font': ['Open Sans Bold'], 'text-allow-overlap': true }, paint: { 'text-color': '#ffffff', 'text-halo-color': '#07111f', 'text-halo-width': 1.5 } });
    const pointLayer = (id, source, color) => map.addLayer({ id, type: 'circle', source, paint: { 'circle-radius': 6, 'circle-color': color, 'circle-stroke-color': '#07111f', 'circle-stroke-width': 2 } });
    pointLayer('incident-teams', 'incident-teams', '#63dcad');
    pointLayer('incident-hazards', 'incident-hazards', '#ff9d4d');
    pointLayer('incident-people', 'incident-people', '#7fd7ff');
    pointLayer('incident-landmarks', 'incident-landmarks', '#dbe7f3');
