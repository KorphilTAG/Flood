      ...
      >
        ...
        <section className="incident-header">
          <div>
            <div className="overline">
              {view === 'command' ? 'INCIDENT COMMAND' : 'FIELD OPERATIONS'} /{' '}
              {mock.areaLabel}
            </div>
          ...
          <div className="incident-data">
            ...
            <div className={operations.offline ? 'warning-text' : ''}>
        ...
        <TabsContent value="command" className="command-view">
          <div className="situation-strip">
            ...
            <span>
              <b>{available.length}</b> teams available
            </span>
            <span>
              <b>
                {
                  mock.sectors.filter(
                    (s) => s.state === 'Unknown' || s.state === 'Stale',
                  ).length
                }
              </b>{' '}
              information gaps
            ...
            <button onClick={() => setActivityOpen(true)}>
              ...
              {operations.reports.length} new field reports
              <ChevronRight size={14} />
            </button>
            <span className="strip-note">
              Estimates require field verification
            </span>
          </div>
          <div className="command-grid">
            <aside className="priority-panel" aria-label="Priority areas">
              ...
              <div className="queue-footer">
                <span>PRIORITY BASIS</span>Life safety · access · information
                gaps
              </div>
            </aside>
            <OperationalMap
                  ...
                  <section>
                    ...
                    {reports.map((r) => (
                      <article key={r.id} className="evidence-item new-report">
                        <StateTag value="Reported" />
                        <p>{r.text}</p>
                        <small>
                          {
                            mock.teams.find((team) => team.id === r.teamId)
                              ?.name
                          }{' '}
                          · {time(r.createdAt)} · {r.confidence} confidence
                        </small>
                        <small>
                          New field report · not independently verified
                        </small>
                      </article>
                    ))}
            ...
            <div className="field-grid">
              <section className="mission-panel">
                ...
                <div className="status-block">
                  ...
                  <p>Updates are shared with Command in this local exercise.</p>
                  <div className="status-buttons">
                    {(
                      ...
                      <button
                        ...
                        className={
                          status === 'Unable to proceed' ? 'unable-button' : ''
                        }
                        onClick={() => {
                          dispatch({
                            type: 'status',
              ...
              <section className="field-report-feed">
                  ...
                  {[...operations.queued, ...operations.reports].filter(
                    ...
                    <div className="empty-state">
                      ...
                      <p>
                        ...
                        progress. New reports appear in Command’s evidence
                        chain.
                      </p>
                    </div>
                  ) : (
                ...
                <div className="field-note">
                  <ShieldCheck size={17} />
                  <p>
                    Session-only prototype. Queued reports are not an offline
                    backup and are lost on reload.
                  </p>
                </div>
              </section>
            </div>
